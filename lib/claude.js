...

{
  ...
  "companyHealthNarrative": "MINIMUM 6-8 sentences -- this is a real company intelligence section, not a 3-sentence summary. Structure: (1) What does ${person.currentCompany} actually do -- products, clients, market, revenue model (2-3 sentences). (2) Where does ${person.currentCompany} sit competitively -- is it a market leader, challenger, niche player? What is the competitive pressure? (2 sentences). (3) What does the workforce data show -- headcount trends, which functions are growing or shrinking, hiring velocity (2-3 sentences). (4) What does this mean for ${person.name} specifically -- is this a stable platform or a volatile one? Is it a company that invests in the ${person.currentTitle} function or one where that function is treated as a cost center? (1-2 sentences).",

  "companyAiExposure": "3-4 sentences on how AI specifically affects ${person.currentCompany}'s business and ${person.name}'s function within it. Is ${person.currentCompany}'s own product or market being disrupted by AI? Is the company adopting AI internally in ways that affect ${person.name}'s team? Is the industry ${person.currentCompany} operates in accelerating or lagging in AI adoption?",
...
export function fallbackNarrative(person, scores, hiringSignals) {
  ...
  return {
    ...
    companyAiExposure: `AI is advancing quickly in ${person.name}'s function — ${dominantFn} — which may affect how ${person.currentCompany} allocates budget and team size in this area. The degree of impact depends on how aggressively ${person.currentCompany}'s industry is adopting AI tooling.`,
