...
function buildFunctionalProfile(jobs) {
  ...
  if (functionalDepth === "deep_specialist") {
    summary = `${Math.round(dominant[1])} years deeply concentrated in ${dominant[0]} (${Math.round(dominantPct * 100)}% of career). Future-proofing paths should respect this functional gravity — lateral moves within or adjacent to ${dominant[0]} are most credible.`;
    ...
    summary = `Career spans multiple functions: ${sorted.slice(0, 3).map(([fn]) => fn).join(", ")}. This breadth suggests adaptability — future-proofing paths can be wider than for a specialist.`;
  } else {
    summary = `True generalist with experience across ${sorted.length} functions. Career pattern suggests comfort with reinvention — future-proofing paths should leverage this adaptability.`;
  }
...
CRITICAL: If the "Current Function" field below says "Unknown", you MUST infer their function from their job title, company, and career history. A "Client Service Manager" is in Client Success / Account Management. A "Marketing Director" is in Marketing. NEVER write "Unknown" in any narrative — always use your inference. If you truly cannot determine function, say "their role" or describe what they do based on their title.

ANTI-SLOP RULES — READ THESE CAREFULLY:
1. NEVER write sentences that could apply to any professional at any company. "The strategic, relationship, and judgment dimensions of the role remain firmly human" is SLOP — it says nothing specific. DELETE sentences like this.
2. Every sentence must contain at least one of: the person's name, their company name, their specific title, a specific industry detail, a specific company detail, or a specific data point. If a sentence contains none of these, it is filler — cut it.
...
Current Function: ${person.currentFunction || "Unknown"} ${(person.currentFunction === "Unknown" || !person.currentFunction) ? `(INFER THIS from their title "${person.currentTitle}" and career history — do not write "Unknown" anywhere in the output)` : ""}
...
Include exactly 4 future-proofing paths ranked by fit. Each path MUST be calibrated to career stage "${careerStage.stage}" — appropriate approaches: ${careerStage.retrainingApproach}.
...
// ─── Infer function from job title when LiveData returns Unknown ─────────
...
// Generates 3+ paragraphs of AI risk analysis — must be specific, not generic
